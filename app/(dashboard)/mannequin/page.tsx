...
import { COLOR_PALETTES, UNDERTONES } from "@/lib/constants";
    ...
    <div className="space-y-10">
        ...
        <div className="bg-cream p-8 border border-tan vintage-border rounded-sm">
          ...
          <div className="flex gap-3">
            {UNDERTONES.map((tone) => (
              <button
                key={tone}
                onClick={() => { setSelectedUndertone(tone); setSelectedPaletteIndex(0); }}
          ...
          <div className="space-y-4">
            {palettes.map((palette, i) => (
              <button
                ...
                className={`w-full text-left bg-parchment p-5 border rounded-sm transition-all ${
                  selectedPaletteIndex === i
                    ? "border-amber shadow-gold"
                    : "border-tan hover:border-amber/40"
                }`}
              >
                <div className="flex items-center justify-between mb-3">
                  <span className="text-base font-body font-bold text-espresso">{palette.name}</span>
                  {selectedPaletteIndex === i && (
                    <div className="w-6 h-6 bg-amber flex items-center justify-center rounded-full">
                ...
                <div className="flex gap-3">
                  {palette.colors.map((color, j) => (
                    <div key={j} className="flex-1 flex flex-col items-center gap-1.5">
                      ...
                      />
                      <span className="text-[10px] text-coffee font-mono">{color}</span>
                    </div>
                  ))}
                </div>
              </button>
      ...
      <ScrollReveal>
        <div className="bg-cream p-10 border border-tan vintage-border rounded-sm">
          <h3 className="text-lg font-display font-bold text-espresso tracking-wider mb-2">MANNEQUIN PREVIEW</h3>
          <p className="text-sm text-coffee font-body mb-8">Dressed in the selected palette: {activePalette.name}</p>
          <div className="flex flex-col md:flex-row items-center gap-10">
            {/* SVG Mannequin */}
            <div className="flex-shrink-0">
              <svg viewBox="0 0 200 500" width="200" height="500" className="drop-shadow-elegant">
                ...
                <path d="M55 88 L145 88 L140 92 L148 160 L150 240 L50 240 L52 160 L60 92 Z" fill={activePalette.colors[0]} stroke="#C4A882" strokeWidth="0.5" />
                <path d="M85 88 L100 110 L115 88" fill="none" stroke="rgba(0,0,0,0.1)" strokeWidth="1" />
                <path d="M55 88 L30 100 L25 190 L35 192 L40 105 L60 92" fill={activePalette.colors[0]} stroke="#C4A882" strokeWidth="0.5" />
                <path d="M145 88 L170 100 L175 190 L165 192 L160 105 L140 92" fill={activePalette.colors[0]} stroke="#C4A882" strokeWidth="0.5" />
                <rect x="50" y="235" width="100" height="8" rx="2" fill="#8B7355" stroke="#C4A882" strokeWidth="0.5" />
                <path d="M50 243 L55 380 L75 385 L95 260 L115 385 L140 380 L150 243 Z" fill={activePalette.colors[1]} stroke="#C4A882" strokeWidth="0.5" />
                ...
                <path d="M55 378 L50 395 L80 398 L78 385" fill={activePalette.colors[2]} stroke="#C4A882" strokeWidth="0.5" />
                <path d="M140 378 L145 395 L115 398 L118 385" fill={activePalette.colors[2]} stroke="#C4A882" strokeWidth="0.5" />
                <circle cx="30" cy="175" r="7" fill={activePalette.colors[3]} stroke="#C4A882" strokeWidth="0.5" />
              </svg>
            </div>

            {/* Color Legend */}
            <div className="flex-1 space-y-3 w-full">
              {mannequinParts.map((outfit) => (
                <div key={outfit.name} className="flex items-center gap-4 bg-parchment p-4 border border-tan rounded-sm card-hover">
                  <div className="w-12 h-12 border border-tan rounded-sm shadow-sm" style={{ backgroundColor: activePalette.colors[outfit.colorIndex] }} />
                  <div>
                    <span className="text-sm font-body font-bold text-espresso">{outfit.name}</span>
                    <p className="text-xs text-coffee font-mono mt-0.5">{activePalette.colors[outfit.colorIndex]}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-cream p-8 border border-tan vintage-border rounded-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(allPaletteColors).map(([key, palette]) => (
              <div key={key} className="bg-parchment p-4 border border-tan rounded-sm card-hover">
                <h4 className="text-sm font-body font-bold text-espresso mb-2">{palette.label}</h4>
                <div className="flex gap-2">
                  {palette.colors.map((color, i) => (
                    <div key={i} className="flex-1 aspect-square border border-tan rounded-sm" style={{ backgroundColor: color }} />
                  ))}
                </div>
              </div>
            ))}
