...
export default function FaceAnalysisError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[Face IQ Error]", error);
  }, [error]);
      ...
      <div className="glass-card max-w-lg w-full p-8 text-center space-y-6">
        ...
        <div>
          <h2 className="type-heading text-[var(--text-primary)] mb-2">
            Face IQ Analysis Error
          </h2>
          <p className="text-sm text-[var(--text-muted)] font-body leading-relaxed">
            {error?.message || "An unexpected error occurred during the face analysis."}
          </p>
          {error?.digest && (
