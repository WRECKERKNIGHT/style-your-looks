...
import { Clock, Trash2, Eye, ArrowRight, GitCompareArrows } from "lucide-react";
    ...
    <div className="space-y-8">
      ...
      {filtered.length === 0 ? (
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="show" className="space-y-2">
          {filtered.map(entry => (
            <div key={entry.id}
              className="glass-card p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 group hover:border-[color-mix(in_srgb,var(--accent-aurum)_40%,transparent)] transition-all">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs text-[var(--text-muted)]">
                    ...
                    {entry.score !== undefined && (
                      <>
                        <span>·</span>
                        <span className="text-[var(--accent-aurum)] font-medium">{entry.score}</span>
                      </>
                    )}
                    {entry.result && (
                      <>
              ...
              <div className="flex items-center gap-2 shrink-0 flex-wrap">
                {entry.type === "face" && (
                  <Link href={`/dashboard/face-comparison?from=${entry.id}`}
                    className="p-2 border border-[var(--border-primary)] rounded-full hover:border-[color-mix(in_srgb,var(--accent-nexus)_50%,transparent)] hover:bg-[color-mix(in_srgb,var(--accent-nexus)_8%,transparent)] transition-all"
                    title="Compare this entry">
                    <GitCompareArrows className="w-3.5 h-3.5 text-[var(--accent-nexus)]" />
                  </Link>
                )}
