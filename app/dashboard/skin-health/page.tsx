...
function getSkinMetrics(faceResult: { skinClarity: number; symmetry: number; overallScore: number } | null): SkinMetric[] {
  ...
  return [
    { label: "Skin Clarity", score: Math.round(clarity * 10) / 10, maxScore: 10, description: "Directly measured from your scan: evenness of brightness across skin regions of the face.", advice: clarity >= 7 ? "Maintain with SPF daily and gentle cleansing." : "Add BHA exfoliant 2x/week and niacinamide serum." },
    { label: "Texture Quality", score: Math.round(texture * 10) / 10, maxScore: 10, description: "Estimated from your measured clarity score — a proxy, not a separate pixel measurement.", advice: texture >= 7 ? "Your texture is smooth. Keep exfoliating regularly." : "Try retinol 2x/week for pore refinement and smoothness." },
    { label: "Tone Evenness", score: Math.round(tone * 10) / 10, maxScore: 10, description: "Derived from your clarity and symmetry measurements — an estimate of color uniformity.", advice: tone >= 7 ? "Your tone is even. Use vitamin C in the morning for brightening." : "Add azelaic acid for hyperpigmentation and use SPF 50 daily." },
    { label: "Hydration Level", score: Math.round(hydration * 10) / 10, maxScore: 10, description: "Inferred estimate based on your skin clarity — photo-based analysis cannot sense moisture directly.", advice: hydration >= 7 ? "Good hydration. Maintain with hyaluronic acid and ceramide moisturizer." : "Add a hydrating toner, hyaluronic acid serum, and thicker night cream." },
    { label: "Elasticity Index", score: Math.round(elasticity * 10) / 10, maxScore: 10, description: "Broad estimate from your overall facial score — treat as directional guidance only.", advice: elasticity >= 7 ? "Good firmness. Protect with antioxidants and sunscreen." : "Add collagen peptides supplement and facial massage routine." },
  ];
}
