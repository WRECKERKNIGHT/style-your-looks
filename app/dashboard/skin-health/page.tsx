...
function getSkinMetrics(faceResult: { skinClarity: number; symmetry: number; overallScore: number } | null): SkinMetric[] {
  ...
  return [
    ...
    { label: "Tone Evenness", score: Math.round(tone * 10) / 10, maxScore: 10, description: "Derived estimate from your clarity and symmetry — color uniformity is inferred, not directly measured.", advice: tone >= 7 ? "Your tone is even. Use vitamin C in the morning for brightening." : "Add azelaic acid for hyperpigmentation and use SPF 50 daily." },
    { label: "Hydration Level", score: Math.round(hydration * 10) / 10, maxScore: 10, description: "Rough estimate inferred from your skin clarity score. Photo-based analysis cannot detect moisture levels.", advice: hydration >= 7 ? "Good hydration. Maintain with hyaluronic acid and ceramide moisturizer." : "Add a hydrating toner, hyaluronic acid serum, and thicker night cream." },
    { label: "Elasticity Index", score: Math.round(elasticity * 10) / 10, maxScore: 10, description: "Derived broadly from your overall facial score. Treat as directional guidance only — not a clinical elasticity reading.", advice: elasticity >= 7 ? "Good firmness. Protect with antioxidants and sunscreen." : "Add collagen peptides supplement and facial massage routine." },
  ];
}
