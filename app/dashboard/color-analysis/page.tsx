...
export default function ColorAnalysisPage() {
  ...
  const exportPaletteCard = useCallback(async () => {
    ...
    svg.innerHTML = `
      ...
      <text x="${w/2}" y="330" text-anchor="middle" font-family="Arial, sans-serif" font-size="9" fill="#C9B18C">Generated by AURAYA · auraya.app</text>
    `;
    ...
    img.onload = () => {
      ...
      link.download = `auraya-palette-${colorAnalysis.subType.toLowerCase().replace(/\s+/g, "-")}.png`;
  ...
  const sharePalette = useCallback(async () => {
    if (!colorAnalysis) return;
    const text = `My AURAYA color palette: ${colorAnalysis.subType}\nBest colors: ${colorAnalysis.bestColors.slice(0, 5).join(", ")}\nMetal preference: ${colorAnalysis.metalPreference}`;
    if (navigator.share) {
      await navigator.share({ title: "My AURAYA Palette", text });
    } else {
